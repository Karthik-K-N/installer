...
func GenerateClusterAPI(ctx context.Context, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID, rhcosImage *rhcos.Image) (*capiutils.GenerateMachinesOutput, error) {
	...
	case "powervs":
		...
		result.Manifests = append(result.Manifests, powervsMachines...)

		image := "capi-powervs-karthik-image"
		bootstrapPowerVSMachine := &capibm.IBMPowerVSMachine{
			...
			Spec: capibm.IBMPowerVSMachineSpec{
				Ignition: &capibm.Ignition{Version: "3.2"},
				SSHKey:   "karthik-ssh",
				//Image: &capibm.IBMPowerVSResourceReference{
				//	Name: &image,
				//},
		...
		result.Manifests = append(result.Manifests, &asset.RuntimeFile{
			File:   asset.File{Filename: fmt.Sprintf("10_inframachine_%s.yaml", bootstrapPowerVSMachine.Name)},
			Object: bootstrapPowerVSMachine,
		})
		...
		bootstrapMachine := &capi.Machine{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name: bootstrapPowerVSMachine.Name,
				Labels: map[string]string{
					"cluster.x-k8s.io/control-plane": "",
				},
			},
