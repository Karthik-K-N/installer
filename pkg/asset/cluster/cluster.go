...
import (
	...
	manifests "github.com/openshift/installer/pkg/asset/manifests/clusterapi"
...
func (c *Cluster) Generate(parents asset.Parents) (err error) {
	...
	switch platform {
	case typesaws.Name, typesazure.Name, typepowervs.Name:
...
func (c *Cluster) generateTerraform(installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID, terraformVariables *TerraformVariables, platform string) error {
	...
	provider, _ := infra.ProviderForPlatform(platform, fg)
...
func (c *Cluster) generateClusterAPI(parents asset.Parents, installConfig *installconfig.InstallConfig, clusterID *installconfig.ClusterID) error {
	...
	if err := capiSystem.Run(context.Background(), installConfig); err != nil {
		return errors.Wrap(err, "failed to run cluster api system")
	}

	// Grab the client.
	cl := capiSystem.Client()
	for _, m := range cluster.RuntimeFiles() {
		m.Object.SetNamespace(capiutils.Namespace)
		if err := cl.Create(context.Background(), m.Object); err != nil {
			return fmt.Errorf("failed to create manifest: %w", err)
		}
		logrus.Infof("Created manifest %+T, namespace=%s name=%s", m, m.Object.GetNamespace(), m.Object.GetName())
	}
